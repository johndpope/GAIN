...
class SEC():
    ...
    def train(self,base_lr,weight_decay,momentum,batch_size,epoches):
        gpu_options = tf.ConfigProto(gpu_options=tf.GPUOptions(per_process_gpu_memory_fraction=0.49))
...
if __name__ == "__main__":
    ...
    sec = SEC({"data":data,"batch_size":batch_size,"input_size":input_size,"epoches":epoches,"category_num":category_num,"model_path":"{}/norm-56999".format(SAVER_PATH),"accum_num":16,"lr_path":"{}/lr-0.000100-52900".format(SAVER_PATH)}) # this for continuing train after interupting

    sec.train(base_lr=1e-3,weight_decay=5e-5,momentum=0.9,batch_size=batch_size,epoches=epoches)
