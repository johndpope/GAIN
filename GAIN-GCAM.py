...
class GAIN():
    ...
    def build_sp_softmax(self, last_layer, is_exist=False, axis=1, layer_name="fc8-softmax"): # SEC
        ...
        preds_exp = tf.exp(self.net[last_layer]-preds_max) + self.clip_eps
        self.net[player] = preds_exp/tf.reduce_sum(preds_exp,axis=1, keepdims=True) + self.min_prob
    ...
    def get_weights_and_bias(self, layer, is_exist=False):
        ...
        if layer.startswith("fc"):
            ...
            elif layer == "fc8": shape=[self.fc8_dim,self.category_num]
    ...
    def optimize(self, base_lr, momentum, weight_decay):
        ...
        self.loss["mask_reg"] = self.get_mask_reg()
    ...
    def train(self, base_lr, weight_decay, momentum, batch_size, epoches, gpu_frac):
        ...
        with self.sess.as_default():
            ...
            while epoch < epoches:
                ...
                if i % 5*iterations_per_epoch_train == 5*iterations_per_epoch_train-1:
                    new_lr /= 10.0
                ...
                params = {self.net["input"]:data_x, self.net["label"]:np.array(data_y).astype(np.float32), self.net["drop_prob"]:0.5}
    ...
    def calc_mask(self, score, eps):
        ...
        probs = scores_exp/(np.sum(scores_exp, axis=2, keepdims=True)+self.clip_eps) + +self.clip_eps
